...
node _dev/link-questions.js         # 全量重刷（242 题 / 399 处）
node _dev/build-search.js --check   # assets/search-index.js 是否该重新生成
node _dev/build-search.js           # 68 页 / 346 个章节 / 约 137 KB
```
...
# 改了 h2 标题里写的题号（即改了题号落点，见下面「题号落点」一节）
node _dev/link-questions.js && node _dev/add-toc.js && node _dev/build-search.js
#   三件事各管一摊、缺一不可：链接的锚点、页内目录的条目、搜索索引的章节名。
#   落点锚点不用手改，link-questions 从标题现算。
...
`checklinks.js` 会顺带把生成的 399 条链接全验一遍**含跨页锚点**，等于白捡一层回归。

### 题号落点 · 链接落到「哪一节」

`……/03-vector-store-and-index.html#s5` 里的 `#s5` 是**落点**：点 `JR-7` 直接到答它的那一节，
而不是材料页顶部。全站 399 条题号链接**全部带锚点**——这条以前是断的（0 条带锚点）。

落点分两层，都由 `qids.js` 的 `buildLandings()` 推导，**真值只有一个：h2 标题里写的题号**：

| 层 | 判据 | 落在哪 | 今天 |
|---|---|---|---|
| 一层（精确） | 题号写在该页某个 h2 标题里（`1. 用户量怎么答（ML-11）`） | 那一节的 `#id` | 230 个题号 |
| 二层（兜底） | 标题里没写 | 该页「讲解骨架」那一节（09 那页认「完整参考回答」） | 12 个题号 |

这个约定**不是这次发明的**——22 页早就这么写标题了，这次只是把它读出来当锚点用，
再把剩下 26 页补齐。**二层兜底不是欠账**：讲解骨架就是这一页所有题的答法，
对「整页才是答案」的概念页（`01-what-is-rag` 那类）来说它就是正确答案——
那一页骨架的第一条口述题恰好就是它认领的 `JR-5`，落过去就是答案本身。
剩下 12 个走兜底的，每一个都有写下来的理由（概念页 / 复合题跨两节 / 经历题全页才答），
**别当待办清单去清零**。

同一个题号写在**两节**标题里是允许的（复合题，如 `YC-5`「怎么拆模块？数据怎么流转？技术选型怎么做？」，
两节各答一部分），落点取文档顺序里第一个。起初这里当成错误抛，磁盘上立刻撞到反例——
**强行只许写一处，等于逼作者少声明「这节也答了这题」，是拿检查器去扭曲内容。**

`buildLandings()` 顺带把三条不变量查了（都在写盘前）：每个被认领的题号都能解析出落点、
写了题号的 h2 有 `id`、标题里的题号属于本页 `.qbadges`。最后一条捞到过真的硬伤：
`02-self-assessment` 的「优点怎么答（FN-8、**FN-9**）」里 `FN-9` 是别页的题。

> **锚点「存在」不用自己查**：`checklinks.js:65-67` 已经在验 `href` 里的 `#fragment`
> 在目标页 `id` 里存在（已用故意改错锚点验证它会报 `[锚点 #NOPE 不存在]`）。
> 它只查存在、不查唯一，和上面三条不重复。
>
> 顺带一个洞：`checklinks.js:58` 对**纯页内**链接（`href="#..."`）直接 `continue`，
> 页内锚点完全不校验。题号链接是**跨页**的（`../step-1-foundation/…html#s5`），
> 走的是被校验的那条路，所以主安全网成立——但往页面里加纯页内锚点时别指望它。

**改 h2 标题会牵动两处派生物**，改完必须按顺序补跑（否则 `--check` 报漂移）：
`add-toc.js`（页内目录是各页 h2 生成的）→ `build-search.js`（搜索索引的章节名同理）。
...
- `qids.js` — 题号的唯一判据：什么算题号、一页认领了哪些、一页声称有多少题、
  **一个题号落在哪一节**（`buildLandings()`）。`link-questions.js` 和 `add-crosswalk.js`
  都读它，上面那三条边和题号落点共用一份口径
